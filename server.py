...
def delete_from_cloudinary(photo_id: str):
    ...
    try:
        ...
        logging.warning(f"Cloudinary delete failed for {photo_id}: {e}")
...
async def list_current_photos(authorization: Optional[str] = Header(None)):
    ...
    return {
        "week_id": wid,
        "photos": [{"photo_id": p["photo_id"], "image_url": p.get("image_url", ""), "caption": p.get("caption", ""), "is_mine": p["photo_id"] in mine_ids} for p in photos],
        "voted_photo_id": vote["photo_id"] if vote else None
    }
...
async def get_photo(photo_id: str, authorization: Optional[str] = Header(None)):
    ...
    result = {
        ...
        "user_voted_photo_id": vote["photo_id"] if vote else None
    }
...
async def replace_photo(photo_id: str, body: PhotoCreate, authorization: Optional[str] = Header(None)):
    ...
    await db.photos.update_one(
        {"photo_id": photo_id},
        {"$set": {"image_url": image_url, "caption": body.caption or "", "updated_at": now_utc()}}
    )
...
async def week_results(week_id: str, authorization: Optional[str] = Header(None)):
    ...
    for p in photos:
        ...
        items.append({
            ...
            "author_picture": author.get("picture", "") if author else ""
        })
